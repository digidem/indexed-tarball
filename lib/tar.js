...
class NoFilesError extends CustomError {
  constructor (message) {
    super(message)
    this.noFiles = true
  }
...
class MalformedError extends CustomError {
  constructor (message) {
    super(message)
    this.malformed = true
  }
...
function readFinalFile (fd, size, cb) {
  ...
  function next (offset) {
    if (offset < 0) return cb(new NoFilesError('No files found.'))
    ...
    fs.read(fd, header, 0, 512, offset, function (err, size, buf) {
      ...
      if (ustarExpected.equals(buf.slice(257, 257 + 5))) {
        ...
        if (isNaN(fileSize)) return cb(new MalformedError('Malformed file'), null, offset, fileName)
