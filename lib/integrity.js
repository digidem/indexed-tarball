...
function repairTarball (filepath, cb) {
  ...
  function checkFinalFile () {
    fs.open(filepath, 'r+', function (err, fd) {
      ...
      tar.readFinalFile(fd, fileLen512 + 1024, function (err, buf, offset, name) {
        ...
        var endOfIndex = offset + buf.length + (512 - buf.length % 512) + 512 * 3
  ...
  function regen (fd, offset, cb) {
    fs.ftruncate(fd, offset, function (err) {
      ...
      buildIndex(fd, offset, function (err, safeLength, index) {
        if (err) return cb(err)
        writeIndex(fd, offset, index, function (err, len) {
          if (err) return cb(err)
          writeTrailer(fd, offset + len, cb)
        })
      })
    })
  }
  ...
  function writeTrailer (fd, offset, cb) {
    ...
    fs.read(fd, trailer, 0, 1024, offset, function (err, bytesRead) {
      if (err) return cb(err)
      // what if we do a partial read but the rest of the bytes are zerod already!?
      if (bytesRead === 1024 && trailer.equals(NUL_TRAILER)) return cb()
...
function buildIndex (fd, fileLen, cb) {
  ...
  ;(function next (pos) {
    checkFile(fd, pos, function (err, header) {
      ...
      var newPos = pos + header.size + leftover + 512

      // only add to the index if the file is long enough! (it might be truncated)
